...
def limpiar_expr(expr):
    if not expr:
        return "0"
    expr = expr.replace("π","pi").replace("\\theta","theta").replace("\\phi","phi")
    expr = expr.replace("\n","").replace("$","")
    ...
    try:
        ...
        for var in orden:
            ...
            if var in limites:
                ...
                paso = integrate(current, symbols(var))
                pasos.append(f"\\textbf{{Integrando respecto a {var} (sin límites):}} \\\\ "
                             f"$\\int {latex(current)} \\, d{var} = {latex(paso)}$")
                current = paso
